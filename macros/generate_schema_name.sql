{#
    ...
    The `tuva_schema_prefix` variable is applied centrally here so that models
    (and folder-level `+schema` defaults in dbt_project.yml) only need to declare
    their bare schema name (e.g. `core`). When `tuva_schema_prefix` is set the
    resolved schema becomes `<prefix>_<schema>`; otherwise it is the bare schema.
#}
