...
def run(spark, enable_normalize_engine=True):
    """Normalize staging: unmapped immunization codes.
    ...
    """
    i = spark.table("core__immunization")
    ...
    if enable_normalize_engine:
        ...
        result = (
            ...
            .filter(
                F.col("i.normalized_code").isNull()
                & F.col("i.normalized_description").isNull()
                & ~(
            ...
            .select(
                ...
                "source_description",
                "item_count",
                F.lit("immunization").alias("domain"),
                "data_source",
            )
        )
    else:
