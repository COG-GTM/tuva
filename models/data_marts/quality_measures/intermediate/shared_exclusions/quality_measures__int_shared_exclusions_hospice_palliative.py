...
def run(spark: SparkSession) -> DataFrame:
    """
    Denominator exclusions for hospice and palliative care.
    """
    exclusion_codes = (
        ...
        .select(
            F.col("code"),
            F.when(F.col("code_system") == "SNOMEDCT", F.lit("snomed-ct"))
            .when(F.col("code_system") == "ICD9CM", F.lit("icd-9-cm"))
            .when(F.col("code_system") == "ICD10CM", F.lit("icd-10-cm"))
            .when(F.col("code_system") == "CPT", F.lit("hcpcs"))
            .when(F.col("code_system") == "ICD10PCS", F.lit("icd-10-pcs"))
            .otherwise(F.lower(F.col("code_system")))
            .alias("code_system"),
            F.col("concept_name"),
        )
    )
    ...
    from_conditions = condition_exclusions.select(
        F.col("person_id"),
        F.col("recorded_date").alias("exclusion_date"),
        F.col("concept_name").alias("exclusion_reason"),
    )

    from_med_claims = med_claim_exclusions.select(
        F.col("person_id"),
        F.coalesce(F.col("claim_end_date"), F.col("claim_start_date")).alias("exclusion_date"),
        F.col("concept_name").alias("exclusion_reason"),
    )

    from_observations = observation_exclusions.select(
        F.col("person_id"),
        F.col("observation_date").alias("exclusion_date"),
        F.col("concept_name").alias("exclusion_reason"),
    )

    from_procedures = procedure_exclusions.select(
        F.col("person_id"),
        F.col("procedure_date").alias("exclusion_date"),
        F.col("concept_name").alias("exclusion_reason"),
    )

    patients_with_exclusions = (
        from_conditions
        .unionByName(from_med_claims)
        .unionByName(from_observations)
        .unionByName(from_procedures)
    )

    result = patients_with_exclusions.select(
        F.col("person_id"),
        F.col("exclusion_date"),
        F.col("exclusion_reason"),
        F.lit("hospice_palliative").alias("exclusion_type"),
